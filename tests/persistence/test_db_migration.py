...
def test_migrate_db_detail(mocker):
    ...
    decision_snapshot_session = MagicMock()
    decision_snapshot_session.scalars.return_value = [decision_snapshot]
    ...
    mocker.patch.object(DecisionSnapshot, "session", decision_snapshot_session, create=True)
    ...
    session_target.add.assert_any_call(decision_snapshot)
