...
class CommittedTemplatePublication(FrozenPlatformModel):
    ...
    def validate_committed_payload(self) -> "CommittedTemplatePublication":
        ...
        try:
            decoded_template = AdapterTemplate.model_validate(
...
class SqlTemplateRepository:
    ...
    def publish_template(
        ...
        template = committed_template.template
        try:
            with Session(self._engine, expire_on_commit=False) as session, session.begin():
                ...
                if existing is not None:
                    return self._resolve_existing(existing, committed_template.payload_digest)
                record = AdapterTemplateRevisionRecord(
                    adapter_template_revision_id=f"template-{committed_template.payload_digest}",
                    template_id=template.template_id,
                    semantic_version=template.semantic_version,
                    canonical_payload=committed_template.canonical_payload,
                    payload_digest=committed_template.payload_digest,
                    source_commit=committed_template.source_commit,
                    root_commit=committed_template.root_commit,
                    backend_commit=committed_template.backend_commit,
                    frontend_commit=committed_template.frontend_commit,
                    strategies_commit=committed_template.strategies_commit,
