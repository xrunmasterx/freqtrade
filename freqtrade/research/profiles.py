...
def load_research_profiles(config: dict[str, Any]) -> list[ResearchBotProfile]:
    ...
    research_input_root = _resolve_research_input_root(config.get("research_input_root"))
    profiles = []
    ...
    for index, raw_profile in enumerate(config.get("research_bots", [])):
        ...
        try:
            market_data = (
                ResearchMarketDataConfig(**raw_market_data) if raw_market_data is not None else None
            )
        except (TypeError, ValidationError) as e:
            raise ResearchConfigError(
        ...
        try:
            side_data = (
                ResearchSideDataConfig(**raw_side_data) if raw_side_data is not None else None
            )
        except (TypeError, ValidationError) as e:
            raise ResearchConfigError(
        ...
        try:
            profiles.append(
                ResearchBotProfile(
                    ...
                    side_data=side_data,
                    data_root=(
                        _resolve_research_input_child(
                            research_input_root,
                            data_source.root,
                            f"{profile_location}.data_source.root",
                        )
                        if research_input_root is not None
                        else user_data_dir / data_source.root
                    ),
                    market_data_root=(
                        (
                            _resolve_research_input_child(
...
def _resolve_research_input_root(configured_root: object) -> Path | None:
    if configured_root is None:
        return None
    try:
        input_root = Path(configured_root)
    except TypeError as e:
        raise ResearchConfigError("Invalid research_input_root") from e
    if not input_root.is_absolute():
        raise ResearchConfigError("Invalid research_input_root")
    return input_root.resolve()


def _resolve_research_input_child(
    input_root: Path,
    configured_root: str,
    location: str,
) -> Path:
    candidate_root = Path(configured_root)
    if ".." in candidate_root.parts:
        raise ResearchConfigError(f"Invalid {location}")
    resolved_root = (
        candidate_root.resolve()
        if candidate_root.is_absolute()
        else (input_root / candidate_root).resolve()
    )
    if not resolved_root.is_relative_to(input_root):
        raise ResearchConfigError(f"Invalid {location}")
